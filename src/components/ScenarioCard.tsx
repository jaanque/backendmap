...
export default function ScenarioCard({ scenario, progress, isFavorited, onToggleFavorite, showFavoriteButton = true }: ScenarioCardProps) {
  ...
  return (
    <div className="block group relative">
       <Link to={`/map/${scenario.slug}`} className="block">
        <article className="border border-zinc-200 rounded-xl p-6 bg-white hover:border-zinc-400 hover:shadow-md transition-all duration-200 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex-grow">
            ...
            <p className="text-zinc-500 text-sm leading-relaxed max-w-2xl pr-8">{scenario.description || 'No description available.'}</p>
